...
### dify_template_workflow_multi_output.yml（マルチ成果物ジェネレーター）

8ノード・10エッジのファンアウト型ワークフロー。
1つの入力から4種類の成果物を同時並列で生成する。

```
開始 → 情報抽出・分析 → ┬→ 週次レポート（マークダウン形式）─┐
                         ├→ メール下書き（上司報告用）────┤
                         ├→ チャット投稿文（Teams/Slack用）─┤→ 統合出力 → 終了
                         └→ アクションアイテム（WBS転記用）─┘
```

- 4本の分岐は同時並列で実行される（Difyが自動で並列処理）
- 統合出力ノードが4つの成果物をまとめて1つの出力にする
- Start変数: question（テキスト、必須）、file（ファイル、任意）
- インポート後にやること: 6つのLLMノードすべてでモデルを選択する
- DifyAppShareでの登録時: アプリタイプ「ワークフロー」を選択
- 注意: LLM6ノード（並列4本+前後2本）のため、実行に40秒〜1分半程度かかる

プロジェクト全体の資料を添付ファイルとして渡すと、最初の「情報抽出・分析」ノードが
メモと資料の両方を読み取って分析する。ナレッジベースを使いたい場合は、Dify上で
インポート後に「知識検索」ノードを追加し、情報抽出ノードのcontextに接続する。

テスト用サンプル入力は report_generator テンプレートのものがそのまま使える。
